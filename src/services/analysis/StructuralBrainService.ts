...
import { CodeGraphBuilder } from '../../utils/CodeGraphBuilder.js';
  ...
  public async scanProject(srcDir?: string): Promise<GodNode[]> {
    ...
    for (const d of candidateDirs) {
      if (d && fs.existsSync(d)) {
        const found = CodeGraphBuilder.findTypeScriptFiles(d);
        found.forEach(f => tsFilesSet.add(f));
      }
    }

    const tsFiles = Array.from(tsFilesSet);
    const importGraph = CodeGraphBuilder.buildImportGraph(tsFiles, this.projectRoot);
