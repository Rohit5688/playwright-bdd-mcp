...
export class TestRunnerService {
    configService;
    envManager;
    constructor(configService, envManager) {
        this.configService = configService || new McpConfigService();
        this.envManager = envManager || new EnvManagerService();
    }
    async runTests(projectRoot, specificTestArgs, timeoutMs, executionCommand) {
        ...
        try {
            ...
            for (const cmdStr of commandSegments) {
                ...
                let exe = parts.shift();
                if (!exe)
                    throw McpErrors.invalidExecutable(cmdStr);
                ...
                const { value: execResult } = await withRetry(() => execFileAsync(exe, args, {
                    cwd: projectRoot,
                    timeout: runTimeout,
                    env: mergedEnv,
            ...
            return {
                passed: false,
                output: `[FAILED] Tests failed or failed to compile.\n\nCommand Error:\n${msg}\n\nStandard Output:\n${error?.stdout || ''}\n\nStandard Error:\n${error?.stderr || ''}` + ExtensionLoader.loadExtensionsForPrompt(projectRoot)
            };
        }
    }
}
