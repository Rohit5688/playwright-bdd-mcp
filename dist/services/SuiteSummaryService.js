...
export class SuiteSummaryService {
    ...
    findFeatureFiles(dir) {
        ...
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                results.push(...this.findFeatureFiles(full));
    ...
    parseFeatureFile(content) {
        ...
        for (const line of lines) {
            ...
            if (trimmed.startsWith('@')) {
                ...
                pendingTags.forEach(t => { if (!tags.includes(t))
                    tags.push(t); });
                pendingTags = [];
            }
            else {
    ...
    buildPlainEnglish(projectRoot, features, totalScenarios, tagBreakdown) {
        ...
        if (smokeCount > 0)
            lines.push(`   npx playwright test --grep @smoke      (${smokeCount} tests)`);
        if (regressionCount > 0)
            lines.push(`   npx playwright test --grep @regression (${regressionCount} tests)`);
        if (e2eCount > 0)
            lines.push(`   npx playwright test --grep @e2e         (${e2eCount} tests)`);
        return lines.join('\n');
    }
}
//# sourceMappingURL=SuiteSummaryService.js.map