...
const domInspectionCache = new Map();
...
OUTPUT INSTRUCTIONS: Do NOT repeat file path or parameters. Do NOT summarise what you just did. Acknowledge in <=10 words, then proceed. Keep response under 100 words unless explaining an error.`,
    ...
    {
        const { testDescription, projectRoot, customWrapperPackage, baseUrl, domJsonContext, testContext: testContextJson } = args;
        await maintenance.ensureUpToDate(projectRoot);
        const config = mcpConfig.read(projectRoot);
        const resolvedWrapper = customWrapperPackage || config.basePageClass;
...
--- END CUSTOM WRAPPER SELECTOR RULES ---`;
        ...
        const resolvedDomContext = domJsonContext ||
        ...
        let resolvedTestContext;
        if (testContextJson) {
            try {
                resolvedTestContext = JSON.parse(testContextJson);
            }
            catch {
                // Malformed JSON — silently ignore, treat as absent
            }
        }
        const instruction = await generator.generatePromptInstruction(testDescription, projectRoot, lastAnalysisResult, resolvedWrapper, baseUrl, memoryPrompt, resolvedDomContext, resolvedTestContext);
...
server.registerTool("gather_test_context", {
...
OUTPUT: JSON string of TestContext. Pass it verbatim to generate_gherkin_pom_test_suite as the testContext parameter.`,
    ...
    const { baseUrl, paths, storageState, loginMacro } = args;
    try {
...
OUTPUT INSTRUCTIONS: Do NOT repeat file path or parameters. Do NOT summarise what you just did. Acknowledge in <=10 words, then proceed. Keep response under 100 words unless explaining an error.`,
    inputSchema: z.object({
        ...
        "returnFormat": z.enum(["markdown", "json"]).optional().describe("Output format. 'markdown' (default) returns Actionable Markdown for LLM prompts. " +
    ...
    {
        const { url, waitForSelector, storageState, includeIframes, loginMacro, projectRoot, returnFormat } = args;
...
server.registerTool("analyze_trace", {
...
OUTPUT INSTRUCTIONS: Do NOT repeat file path or parameters. Do NOT summarise what you just did. Acknowledge in <=10 words, then proceed. Keep response under 100 words unless explaining an error.`,
    ...
    {
        const { projectRoot, traceFile } = args;
...
OUTPUT INSTRUCTIONS: Do NOT repeat file path or parameters. Do NOT summarise what you just did. Acknowledge in <=10 words, then proceed. Keep response under 100 words unless explaining an error.`,
...
}, async (args) => {
    {
        ...
        const jsonSteps = args.jsonSteps;
        ...
        try {
            ...
            for (const f of files) {
                if (!f.path.endsWith('.ts'))
                    continue; // only TypeScript POM files
            ...
            for (const f of files) {
                ...
                if (classMatches.length > 1) {
                    ...
                    monolithicViolations.push(`  📄 ${f.path} — contains ${classMatches.length} classes: [${classNames}]`);
                }
            }
