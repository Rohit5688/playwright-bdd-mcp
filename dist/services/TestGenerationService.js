...
export class TestGenerationService {
    async generatePromptInstruction(testDescription, projectRoot, analysisResult, customWrapperPackage, baseUrl, memoryPrompt = "", domJsonContext, // Optional: JSON string of JsonElement[] from inspect_page_dom(returnFormat:'json')
    testContext) {
   ...
   Direct \`this.page.*\` calls are ONLY allowed for: \`this.page.getByRole()\`, \`this.page.getByTestId()\`, \`this.page.context()\`, \`this.page.route()\`, \`this.page.waitForResponse()\` (inside \`Promise.all\`), multi-tab handling.`
        ...
        if (domJsonContext) {
            let elements = [];
            try {
                elements = JSON.parse(domJsonContext);
            }
