 ...
 */
const REDACTION_RULES: Array<{ pattern: RegExp; replacer: string }> = [
 ...
 */
export function validateProjectPath(projectRoot: string, relativePath: string): string {
  ...
  if (!resolvedPath.startsWith(normalizedRoot + path.sep) && resolvedPath !== normalizedRoot) {
    throw McpErrors.permissionDenied(
      resolvedPath,
      `PATH SECURITY VIOLATION: Resolved path "${resolvedPath}" escapes the project root "${normalizedRoot}".\n` +
 ...
 */
const SECRET_AUDIT_RULES: Array<{ pattern: RegExp; description: string; safeAlternative: string }> = [
  {
 ...
 */
export function auditGeneratedCode(files: AuditableFile[]): string[] {
    ...
    for (const rule of SECRET_AUDIT_RULES) {
      ...
      if (matches) {
        for (const match of matches) {
          violations.push(
            `⚠️ SECRET DETECTED in "${file.path}": ${rule.description}\n` +
 ...
 */
export function safeJsonParse<T = unknown>(text: string, label?: string): T | null {
  try {
