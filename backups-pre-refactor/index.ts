...
(server as any).registerTool = (name: string, info: any, handler: any) => {
  const wrappedHandler = async (args: any, extraOptions?: any) => {
    ...
    try {
      ...
      let outputSummary: Record<string, any> = {};
      if (result) {
        outputSummary = {
...
function getNavService(projectRoot: string): NavigationGraphService {
...
--- END CUSTOM WRAPPER SELECTOR RULES ---`;
      ...
      if (!resolvedDomContext && !resolvedTestContext) {
        throw McpErrors.projectValidationFailed(
          "DOM PRE-FLIGHT REQUIRED: You must inspect the page before generating a test suite. Call 'gather_test_context' (for multi-page flows) OR 'inspect_page_dom(returnFormat: json)' (for single pages) first, then pass the result to this tool.",
          "generate_gherkin_pom_test_suite"
        );
      }
...
OUTPUT INSTRUCTIONS: Do NOT repeat file path or parameters. Do NOT summarise what you just did. Acknowledge in <=10 words, then proceed. Keep response under 100 words unless explaining an error.`,
  ...
  async (args) => {
    {
      const { errorDna, pageUrl, projectRoot } = args as any;
      let memoryPrompt = '';
      let sessionTimeout = 30000;
      if (projectRoot) {
...
⚠️ PATH WARNINGS:\n${writeResult.warnings.join('\n')}`;
      ...
      try {
        ...
        if (runResult.passed) {
          ...
          selfHealer.resetAttempts(projectRoot);
          ContextManager.getInstance().purgeOldContext();
          return {
            content: [{
              type: "text",
...
OUTPUT INSTRUCTIONS: Do NOT repeat file path or parameters. Do NOT summarise what you just did. Acknowledge in <=10 words, then proceed. Keep response under 100 words unless explaining an error.`,
  ...
  async (args) => {
    {
      const { projectRoot, config: configPatch, credentials, appPath } = args as any;
      ...
      if (operation === "preview") {
        if (!configPatch) throw McpErrors.invalidParameter("config", "\'preview\' action requires a \'config\' object.");
      ...
      if (operation === "set_credentials") {
        if (!credentials) throw McpErrors.invalidParameter("credentials", "'set_credentials' requires a 'credentials' object");
...
OUTPUT INSTRUCTIONS: Do NOT repeat file path or parameters. Do NOT summarise what you just did. Acknowledge in <=10 words, then proceed. Keep response under 100 words unless explaining an error.`,
  ...
  async (args) => {
    {
      const { operation = 'start' } = args as any;
...
OUTPUT INSTRUCTIONS: Do NOT repeat file paths or parameters. Do NOT summarize what you just did. Briefly acknowledge completion (≤10 words), then proceed to next step.`,
  ...
  async (args) => {
    try {
      const result = await orchestrationService.healAndVerifyAtomically(
        args.projectRoot,
        args.error,
        args.xml,
        args.oldSelector,
  ...
  async (args) => {
    {
      ...
      const apiRegistry: SandboxApiRegistry = {
        ...
        inspectPageDom: async (url: string, opts: any = {}) => {
          return await domInspector.inspect(
            url,
        ...
        runTest: async (testPath: string, projectRoot?: string) => {
          const root = projectRoot || process.cwd();
          const config = mcpConfig.read(root);
          return await runner.runTests(root, testPath, config.timeouts?.testRun || 120000);
        },
        readFile: async (filePath: string, projectRoot?: string) => {
          ...
          if (!fs.existsSync(resolvedFile)) { const enhanced = FileSuggester.enhanceError(resolvedFile); throw McpErrors.fileNotFound(enhanced, 'SandboxEngine.readFile'); }
        ...
        parseAST: async (filePath: string, options?: { extractSignatures?: boolean; projectRoot?: string }) => {
          ...
          if (!ts) throw McpErrors.projectValidationFailed("typescript package not available");
